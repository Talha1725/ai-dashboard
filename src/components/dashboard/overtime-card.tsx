  ...
  return (
    <MetricCard title="Weekly Overtime" source={overtime.source} status={overtime.status}>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
        ...
        >
          <div className="grid size-24 place-items-center rounded-full bg-surface text-center shadow-inner">
            <div>
              <div className="text-3xl font-bold text-foreground">{overtime.hours}</div>
              <div className="text-xs font-semibold uppercase tracking-[0.08em] text-text-subtle">
                hours
              </div>
            </div>
          </div>
        ...
        <div>
          <p className="text-sm text-text-soft">{percentage}% of team in overtime</p>
          <p className="mt-2 text-2xl font-bold text-foreground">
            +{formatCurrency(overtime.costImpact)}
          </p>
          <p className="text-sm text-text-subtle">cost impact this week</p>
        </div>
      </div>
    </MetricCard>
  );
